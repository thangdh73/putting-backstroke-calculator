...
class ProfessionalPuttGenerator:
    ...
    # ------- physics helpers -------------------------------------------------
    @staticmethod
    def _to_m(ft: float) -> float:
        return ft * FT_TO_M

    def _velocity(self, dist_ft, stimp, slope_pc):
        d_m = self._to_m(dist_ft)
        v = 0.36 * stimp * (1 - exp(-d_m / 9.5))
        v *= 1 + slope_pc * 0.003
        return float(np.clip(v, self.min_velocity, self.max_velocity))

    def _swing_params(self, bpm, rhythm, dist_ft, stimp, slope_pc):
        dsi = 30 / bpm
        back_t = dsi * rhythm
        base_len_in = 6.5
        scaling = min(0.8 * sqrt(dist_ft / 10), 3.5)
        back_len_in = min(base_len_in * scaling, self.max_backswing_in)
        return {
            "dsi_time": dsi,
            "backswing_time": back_t,
            "backswing_length_in": back_len_in,
    ...
    def generate(self, bpm, rhythm, dist_ft, stimp, slope_pc, handed="right"):
        p = self._swing_params(bpm, rhythm, dist_ft, stimp, slope_pc)
        ...
        pan_b = np.linspace(1, 0, len(back)) if handed == "right" else np.linspace(0, 1, len(back))
        pan_d = 1 - pan_b[::-1]

        L = np.concatenate([back * pan_b, down * pan_d])
        R = np.concatenate([back * (1 - pan_b), down * (1 - pan_d)])
    ...
    # ------- MP3 / WAV export  (graceful fallback) ---------------------------
...
# ───────────────────── Workbook parser + model helpers (unchanged) ──────────
...
def _find_header_row(df: pd.DataFrame) -> int | None:
    for i, v in df.iloc[:, 0].items():
        if _normalise(v).lower() == "putt length (m)":
            return i
    return None
...
def load_workbook(xlsx):
    ...
    for sh in wb.sheet_names:
        ...
        df = wb.parse(sh, header=None); hdr = _find_header_row(df)
...
def _build_from_excel(p):
    df = load_workbook(p); model = build_model(df).fit(
        df[["Putt Length (m)", "Stimp", "Direction", "Elevation (cm)"]], df["Backstroke (cm)"])
    FRAME_PATH.write_bytes(df.to_parquet()); joblib.dump(model, MODEL_PATH); return df, model
...
if st.button("Predict & Play"):
    ...
    # --- audio --------------------------------------------------------------
    dist_ft = putt_m * M_TO_FT
    gen = ProfessionalPuttGenerator()
    L, R, swing = gen.generate(tempo, ratio, dist_ft, stimp_ft, slope, hand.lower())

    if repeat_n > 1:
        L = np.tile(L, repeat_n)
        R = np.tile(R, repeat_n)
...
st.caption("Workbook-driven backstroke model + repeatable SoundTempo tone")
